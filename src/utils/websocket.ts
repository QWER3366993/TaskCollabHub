...
export const initWebSocket = (userId: string, token: string) => {
  ...
  const connect = () => {
    ...
    webSocket.onopen = () => {
      reconnectAttempts = 0;
      chatStore.setOnlineStatus(true);
      startHeartbeat();
    };
  ...
  const handleMessage = (data: ChatMessage) => {
    if (data.system) {
      const contentArray = Array.isArray(data.content) ? data.content : data.content.split(',').map(item => item.trim());

      chatStore.updateSystemMessages(contentArray);
    } else {
